...
function CreateGraph(resX,... % estrutura com resultados dos testes para grandeza X
                     arqX)    % arquivos para guardar os resultados dos testes para grandeza X
yvector = cell2mat([resX.tabFreq(2:end,3)';resX.tabFreq(2:end,9)';...
                    resX.tabFreq(2:end,11)';resX.tabFreq(2:end,13)']');
...
label = ['Erro ' resX.id ' (%)'];
% Axes Labels
xlabel({label},'FontSize',26.4);
ylabel({'% de Carga'},'FontSize',26.4);
...
savepath = [arqX.nomearqfig(1:length(arqX.nomearqfig)-4) '.png'];
saveas(gcf,arqX.nomearqfig);
